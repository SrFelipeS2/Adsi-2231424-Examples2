...
class Cuenta{
    ...
    public function retirarSaldo($valorretiro)
    {
        $this-> saldo = $this-> saldo - $valorretiro;
    }
    
    public function pagarFactura($medio_pago, $pfactura, $numero_de_tarjeta, $numero_de_cuenta){
        if($medio_pago=="SaldoCuenta"){
           if ($this->saldo>=$pfactura && $this -> numero_de_cuenta == $numero_de_cuenta ){
            $this -> saldo= $this-> saldo - $pfactura;
            return true;
           }else{
               return false;
           }
        }elseif($medio_pago=="SaldoTarjeta") {
            if($this->saldo_tarjeta>=$pfactura && $this->numero_tarjeta == $numero_de_tarjeta){
                $this -> saldo_tarjeta= $this-> saldo_tarjeta - $pfactura;
                return true;
            }else{
                return false;
            }
